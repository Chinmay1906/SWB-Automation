...
namespace OptiProERPSWB_WebAutomation
{
    class CommonFunction
    {
        ...
        {
            AutomationTestUI.driver.FindElement(By.Name(psIdName)).SendKeys(psvalue);
        }

        public void SetValueByXpath(string psIdName, string psvalue)
        {
            AutomationTestUI.driver.FindElement(By.XPath(psIdName)).SendKeys(psvalue);
        }

        public void Keypress(string psIdName, string KeyName)
        {
            AutomationTestUI.driver.FindElement(By.XPath(psIdName)).SendKeys(KeyName);
        }
        ...
        {
            AutomationTestUI.driver.FindElement(By.Name(psIdName)).Clear();
        }

        public void ClearValueByXpath(string psIdName)
        {
            AutomationTestUI.driver.FindElement(By.XPath(psIdName)).Clear();
        }
        ...
        {
            AutomationTestUI.driver.FindElement(By.XPath(psIdName)).Click();
        }

        public void Click_Name(string psIdName)
        {
            AutomationTestUI.driver.FindElement(By.Name(psIdName)).Click();
        }
        ...
        public IWebElement GetLookupDataUI(string psIdName)
        {
            IWebElement psReturn;
            psReturn = AutomationTestUI.driver.FindElement(By.Id(psIdName));
           
            return psReturn;
        }

        public IWebElement GetLookupDataUI_xpath(string psIdName)
        {
            IWebElement psReturn;
            psReturn = AutomationTestUI.driver.FindElement(By.XPath(psIdName));
        ...
        // 
        public bool SelectValueFromLookupWithPager(string psValue, IWebElement LookupTable,int ColumID)
        {
            bool pbReturn = true;
            bool pbPagerNextBtnDisabled = false;
            bool pbExist = false;
        ...
        public bool SelectValueFromPopupWithoutpager(string psValue, int ColumID)
        {
            ...
            try
            {
                IList<IWebElement> elements = AutomationTestUI.driver.FindElements(By.XPath("//tr/td[" + ColumID + "]"));
        ...
        public bool ClickOnActionbtn(string psValue, IWebElement UserGridTable, bool pbRowSelect, string paCodeColumID, string piActionColumID, string EditButton, string DeleteButton)
        {
            bool pbReturn = true;
            bool pbExist = false;
            // int Action = 4;//
            ...
            try
            {
                ...
                IList<IWebElement> CodeElements = UserGridTable.FindElements(By.XPath("//tr/td[" + paCodeColumID + "]"));

                IList<IWebElement> Actionelement = UserGridTable.FindElements(By.XPath("//tr/td[" + piActionColumID + "]"));

                for (int i = 0; i < CodeElements.Count; i++)
                {
                    System.Console.WriteLine(CodeElements[i].Text);
                    String psCode = CodeElements[i].Text;

                    if (psValue == psCode)
                    {
                        if (pbRowSelect == false)
                        {
                            Actionelement[i].FindElement(By.XPath(EditButton)).Click();
                            //Actionelement[i].Click();
                            pbExist = true;
                            pbReturn = true;
                            break;
                        }
                        ...
                        {
                            Actionelement[i].FindElement(By.Id(DeleteButton)).Click();
                            pbExist = true;
                            pbReturn = true;
                            break;
                        }
        ...
        public void SetValueIntoGridWithoutPager(IWebElement LookupTable, string FName, string Remark, string Taxcode, string DistribMethod, string NetAmount)
        {
            ...
            for (int i = 0; i < remark.Count; i++)
            {
                if (FName == Name[i].Text)
                {
                    ...
                    //AutomationTestUI.driver.FindElement(By.Id(CommonID.PicklistInput)).SendKeys(Keys.Tab);
                    System.Threading.Thread.Sleep(500);
            ...
            //        IWebElement selectElement = AutomationTestUI.driver.FindElement(By.TagName("select"));
            //        selectElement.SendKeys(DistribMethod);
        ...
        public void SetValueIntoGridWithoutPager_TaxCodeLookup(IWebElement LookupTable, string FName, string Remark, string Taxcode, string DistribMethod, string NetAmount)
        {
            ...
            for (int i = 0; i < remark.Count; i++)
            {
                if (FName == Name[i].Text)
                {
                    ...
                    AutomationTestUI.driver.Manage().Timeouts().ImplicitWait = TimeSpan.FromSeconds(20);
                    AutomationTestUI.objCommonFunction.Click(CommonID.FreightTaxCodeLookupBtn);

                    AutomationTestUI.driver.Manage().Timeouts().ImplicitWait = TimeSpan.FromSeconds(20);
                    IWebElement TaxGridTable = AutomationTestUI.objCommonFunction.GetLookupDataUI(CommonID.TaxCode_grid);
                    AutomationTestUI.driver.Manage().Timeouts().ImplicitWait = TimeSpan.FromSeconds(20);
                    System.Threading.Thread.Sleep(500);

                    ItemcodeLookupExist = AutomationTestUI.objCommonFunction.SelectValueFromLookupWithOutPager(Taxcode, TaxGridTable);
                    if (ItemcodeLookupExist == false) { AutomationTestUI.objCommonFunction.Click(CommonID.TaxCode_grid_closebtn); }
